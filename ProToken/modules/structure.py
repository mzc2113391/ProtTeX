...
class FoldIteration(nn.Module):
    ...
    def setup(self):
        ...
        self.transition_1 = nn.Dense(features=self.config.num_channel, kernel_init=nn.initializers.lecun_normal(), 
                                     dtype=self._dtype, param_dtype=jnp.float32)
        self.transition_2 = nn.Dense(features=self.config.num_channel, kernel_init=nn.initializers.zeros_init(), ### debug zeros_init -> lecun_normal
                                     dtype=self._dtype, param_dtype=jnp.float32)
        self.affine_update = nn.Dense(features=6, kernel_init=nn.initializers.zeros_init(), ### debug zeros_init -> lecun_normal
                                      dtype=self._dtype, param_dtype=jnp.float32)
